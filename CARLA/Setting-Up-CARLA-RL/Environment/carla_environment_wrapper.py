...
class CarlaEnvironmentWrapper(EnvironmentWrapper):
	def __init__(self, num_speedup_steps = 30, require_explicit_reset=True, is_render_enabled=False, early_termination_enabled=False, run_offscreen=False, cameras=['SceneFinal'], save_screens=False):
		...
		self.log_path = os.environ["CARLA_ROOT"]+'/Setting-Up-CARLA-RL/logs/CarlaLogs.txt'
	...
	def _update_state(self):
		...
		if speed_reward > 40.:
			speed_reward = 40.
		self.reward = speed_reward*12 \
					  - (measurements.player_measurements.intersection_otherlane * (self.car_speed+1.5)*12) \
					  - (measurements.player_measurements.intersection_offroad * (self.car_speed+2.5)*150) \
					  - is_collision * 250 \
					  - self.control.brake*200
	...
	def save_screenshots(self):
		...
		'''
		if self.segment_camera:
			save_image(self.segmented_img_path+filename+".png", convert_segmented_to_rgb(carla_config.colors_segment, self.observation['segmented_image'])) # This was self.observation['segmented_image'] earlier
		if self.depth_camera:
