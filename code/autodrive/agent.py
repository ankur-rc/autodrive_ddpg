...
Last Modified: Monday December 3rd 2018 5:46:02 pm
Author: ankurrc
'''
...
class DDPG_PERAgent(Agent):
    """DDPG with prioritized experience replay (PER)
    """
    ...
    def __init__(self, nb_actions, actor, critic, critic_action_input, memory,
        ...
        assert type(memory) is PrioritizedExperience

        super(DDPG_PERAgent, self).__init__(**kwargs)
    ...
    def backward(self, reward, terminal=False):
        ...
        if can_train_either and self.step % self.train_interval == 0:
            beta = self.memory.beta + \
                (1.0 - self.memory.beta)*(self.step/self.nb_steps)
            experiences, is_weights, is_indices = self.memory.sample(
            ...
            if self.step > self.nb_steps_warmup_critic:
                ...
                # prioritized experience replay: importance sampling for weights
                metrics = self.critic.train_on_batch(
                    state0_batch_with_action, targets, sample_weight=is_weights)
                if self.processor is not None:
